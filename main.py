...
async def showall(
    ...
    log("SHOWALL", "Alle Zahlungen abgerufen")
    # Gibt rohe payments-Dict zurück, Timestamps als float
    result = {}
    for sender, zahlungen in payments.items():
        result[sender] = [
            {
                "amount": str(z["amount"]),
                "receiver": z["receiver"],
...
async def reset_payments(
    ...
    if key in payments:
        del payments[key]
        log("RESET", f"✅ Zahlungen von '{r.sender}' gelöscht")
        return {"ok": True, "sender": r.sender}
    else:
        log("RESET", f"'{r.sender}' nicht gefunden")
        raise HTTPException(status_code=404, detail=f"Keine Zahlungen für '{r.sender}' gefunden")
